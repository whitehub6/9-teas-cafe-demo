...
function loadCart() {
  const savedCart = localStorage.getItem('teas_cart');
  if (savedCart) {
    try {
...
function saveCart() {
  localStorage.setItem('teas_cart', JSON.stringify(cart));
}
...
function showToast(message) {
  const toast = document.getElementById('teasToast');
  if (toast) {
...
if (orderForm && checkoutFormStep && checkoutReceiptStep) {
  orderForm.addEventListener('submit', (e) => {
    ...
    checkoutReceiptStep.innerHTML = `
      ...
      <h3>Thank You</h3>
      <p class="lead">Your order proposal has been sent to our cafe in Bhayandar. We will call you to confirm.</p>
      
      <div class="retro-invoice">
        <div class="invoice-header">
          <div class="invoice-title">9 TEA'S CAFE</div>
        ...
        <div class="invoice-details-box">
          ...
          <div>
            ...
            <p>${option === 'delivery' ? address : 'Bhayandar East, Mumbai'}</p>
          </div>
        </div>
...
// Add to Cart event delegator for dynamic elements
